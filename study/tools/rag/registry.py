...
def export_topics_md() -> None:
    ...
    lines = [
        ...
        " primary textbook section, e.g. `3.5` or `3.5, 3.6` |"
    ...
    lines += [
        f"| {t['topic']} | {t['book']} | {t['section']} | {t['status']} | {t['note']} |"
        for t in topics
    ]
...
def add_topic(topic: str, book: str = "", section: str = "", note: str = "") -> None:
    ensure_ready()
    conn = connect()
    conn.execute(
        "INSERT INTO topics(topic, book, section, status, note, created_at, updated_at)"
        ...
        " section=excluded.section, status=excluded.status,"
        " note=excluded.note, updated_at=excluded.updated_at",
        (topic, book, section, "todo", note, _now(), _now()),
    )
    conn.commit()
    conn.close()
    export_topics_md()
...
def update_topic(topic: str, **fields: str) -> bool:
    ensure_ready()
    allowed = {"book", "section", "status", "note"}
...
def list_topics(status: str | None = None, book: str | None = None) -> list[TopicRow]:
    ...
    conn = connect()
    rows = [
        TopicRow(r["topic"], r["book"], r["section"], r["status"], r["note"])
