...
#   API 는 http://<host>:18080 (POST /run-plans, POST /split-plans, GET /health).
#
# 환경(선택):
#   AGENT_MODE=mock   # 오프라인 echo (--live 아님) — 서버 없이 파이프라인만
#   AGENT_PARSER/WORKERS/CODERS/REASONER/EMBED   # 역할 주소 재정의
#   MYLLM_API / MYLLM_TOKEN                      # 모델기동(DOC/1) 클라이언트 (기본 127.0.0.1:8000)
...
info "http://localhost:18080 (POST /run-plans) · 중지하려면: bash server-down.sh"
