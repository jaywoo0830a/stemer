...
def main(argv=None) -> int:
    ...
    # live 이면 다른(논리적) 모델을 판사로 구성해 답 검증 + 출제판사 (offline 은 off)
    vf = None
    pv = None
    if live:
        try:
            from agent.verify import (LlmVerifier, ProblemsVerifier,
                                      pick_judge_server)
            def vf(role):
                try:
            ...
            try:
                u = pick_judge_server("problems", reg)
                pv = ProblemsVerifier(Gateway(base_url=u),
                                      judge_role=u.rpartition(":")[2])
