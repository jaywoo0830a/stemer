...
def problems_correction_prompt(question, chunks, reason) -> str:
    ...
    return (
        ...
        "produced by an executable ```sympy``` block immediately after that "
        "Solution key, and the Solution key's final number MUST equal what that "
        "block prints. A hand-derived or mismatched number (e.g. index slip 45 vs "
        "the correct 32) is rejected automatically. Concept-only problems omit the "
        "block.\n\n"
        "ORIGINAL REQUEST:\n" + question.strip() + "\n\n"
